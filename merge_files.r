...
NMNH_Siphonaptera <- read.csv("~/GitHub/tpt-siphonaptera/output/Lewis_DwC.csv", na = "NA") # read in cleaned Lewis review file
...
FMNH_not_in_NMNH <- anti_join(FMNH_Siphonaptera, NMNH_Siphonaptera, by = "canonicalName") # get all rows in FMNH that does not match a canonical name in Lewis

merged_siphonaptera <- rbindlist(list(NMNH_Siphonaptera, FMNH_not_in_Lewis), fill = TRUE) # add FMNH terms not in Lewis to working file
...
merged_not_in_Lewis <- anti_join(merged_siphonaptera, Lewis_Siphonaptera, by = "canonicalName") # get all rows in merged that do not match a canonical name in Lewis
...
merged_not_in_GBIF <- anti_join(merged_siphonaptera, GBIF_Siphonaptera, by = "canonicalName") # get all rows in merged that do not match a canonical name in GBIF
...
GBIF_not_in_merged <- anti_join(GBIF_Siphonaptera, merged_siphonaptera, by = "canonicalName") # get all rows in GBIF that do not match a canonical name in merged
...
review_dups <- Siphonaptera[duplicated(dupe) | duplicated(dupe, fromLast=TRUE),]
